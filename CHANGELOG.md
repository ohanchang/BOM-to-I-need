...
## 🚀 v1.6：儲存格排版優化、大分類統計雙公式重構與公式修正 (Layout Optimization & Category Formula Refactor)
**時間點：2026-07-04**

### 變更摘要
1. **BOM 頁面元數據儲存格清空與排版優化**：
   - 針對 A 列具有長文字標題的行（BOM元數據區），強制清空其對應的 C 與 D 欄儲存格（C4, D4, C7~C10, D7~D10），確保文字能夠在 Excel 中向右完整顯示而不被遮蔽。
2. **匯率與總金額欄位向右偏移至 F 欄**：
   - 將原本位於 E 欄的 `匯率` 標記、數值與總金額（E2, E3, E4）向右移至 F 欄（F2, F3, F4），G4 換算公式同步調整為 `F4/F3`。F2 標記設為粗體，F4 與 G4 維持粗體紅色樣式。
3. **新增 F6 與 G6 拆解分析總額參照**：
   - BOM 頁面新增 F6 與 G6 欄位。F6 設為公式 `=物料拆解分析!B15`，以參照拆解頁面的計算總額；G6 設為公式 `=F6/F3` 計算美金總額。格式比照 F4/G4 設為粗體紅色、小數點後兩位。
4. **主料缺料狀態分析與樣式 (D2 儲存格)**：
   - 在 D2 儲存格新增智慧型主料缺料狀態顯示：
     - 若無任何缺料：顯示 `主料沒有缺料`。
     - 若僅缺失 `6LL` 開頭的大標料（不限個數）：顯示 `主料只缺大標`。
     - 若缺失兩個以上主料或單個非 6LL 主料：顯示 `主料缺#個料`（# 為實際缺失個數）。
   - **樣式設定**：D2 儲存格同步設定為粗體與紅色，突出顯示缺料狀態。
5. **物料拆解分析 C 欄百分比公式修正**：
   - 將物料拆解分析頁面 C 欄百分比分母由原先錯誤的 `$B$16` 修正為 `$B$15`（Total 所在行數）。
6. **大分類統計雙公式分析重構與簡化**：
   - 重構「大分類統計」工作表，將原本 B 欄的 `SUMIFS` 替換為參照「物料拆解分析」設計的 `SUMPRODUCT` 陣列分類公式，C 欄對應計算 B 欄百分比。
   - 依據最新需求，移除了用於對照的 `SUMIFS` 金額及百分比欄位（原 E、F 欄與 D 欄空白分隔），僅保留採用 `SUMPRODUCT` 新公式的 A, B, C 三欄，維持頁面清爽。
7. **DATA 目錄忽略設定**：
   - 於 `.gitignore` 檔案中新增忽略 `DATA/` 資料夾，以防測試範本與輸出樣本檔案被同步至程式碼儲存庫中。
