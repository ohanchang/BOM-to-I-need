...
| 15 | 功能 | 導出打包檔案時新增 `Missing Price List.xlsx` 彙整缺價主料 | ✅ 完成 |
| 16 | UI | 檔案卡片右上角新增缺主料警示標籤與數量顯示 | ✅ 完成 |
| 17 | UI / 格式 | BOM 元數據特定儲存格清空防遮蔽與欄位（匯率、總價）右移 F 欄排版優化 | ✅ 完成 |
| 18 | 功能 | 大分類統計雙公式（SUMPRODUCT & SUMIFS）對比分析重構與物料拆解百分比分母修正 | ✅ 完成 |
| 19 | 運作安全 | `.gitignore` 配置忽略本地 `DATA/` 目錄防測試資料同步 | ✅ 完成 |
